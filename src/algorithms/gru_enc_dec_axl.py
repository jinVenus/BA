...
class GRUED(Algorithm, PyTorchUtils):
    def __init__(self, name: str = 'GRU-ED', num_epochs: int = 20, batch_size: int = 20, lr: float = 1e-3,
    ...
    def fit(self, X: pd.DataFrame, seq=None):
        ...
        self.optimizer = torch.optim.Adam(self.grued.parameters(), lr=self.lr)
        ...
        for epoch in trange(self.num_epochs):
            ...
            for ts_batch in train_loader:
                ...
                for ts_batch_val in train_loader:
                    output = self.grued(self.to_var(ts_batch_val))
