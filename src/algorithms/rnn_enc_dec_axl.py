...
class RNNED(Algorithm, PyTorchUtils):
    def __init__(self, name: str = 'RNN-ED', num_epochs: int = 20, batch_size: int = 20, lr: float = 1e-3,
    ...
    def fit(self, X: pd.DataFrame, seq=None):
        ...
        self.optimizer = torch.optim.Adam(self.rnned.parameters(), lr=self.lr)
        ...
        for epoch in trange(self.num_epochs):
            ...
            for ts_batch in train_loader:
                ...
                for ts_batch_val in train_loader:
                    output = self.rnned(self.to_var(ts_batch_val))
    ...
    def predict(self, X: pd.DataFrame, seq = None, update = None, t = None, data = None):
        ...
        if update:
            ...
            # data = X.values
            data = X
            index = np.arange(0, data.shape[0] - self.sequence_length + 1, 200, int)
        ...
        lattice = np.full((self.sequence_length - 200 + 1, data.shape[0]), np.nan)
        # lattice = np.full((2, data.shape[0]), np.nan)
        for i, score in enumerate(scores):
            lattice[i % self.sequence_length - 200 + 1, i * 200:i * 200 + self.sequence_length] = score
            # lattice[i % 2, i * 25:i * 25 + self.sequence_length] = score
        scores = np.nanmean(lattice, axis=0)

        if self.details:
            outputs = np.concatenate(outputs)
            lattice = np.full((self.sequence_length - 200 + 1, X.shape[0], X.shape[1]), np.nan)
            for i, output in enumerate(outputs):
                lattice[i % self.sequence_length - 200 + 1, i * 200:i * 200 + self.sequence_length, :] = output
            ...
            for i, error in enumerate(errors):
                lattice[i % self.sequence_length - 200 + 1, i * 200:i * 200 + self.sequence_length, :] = error
...
class RNNEDModule(nn.Module, PyTorchUtils):
    ...
    def forward(self, ts_batch, return_latent: bool = False):
        ...
        # 2. Use hidden state as initialization for our Decoder-RNN
